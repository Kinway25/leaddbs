...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr  7 17:48:06 2026
...
def load_from_json(axon_parameters_file,hemi_idx,merge=True):
    with open(axon_parameters_file, 'r') as fp:
        d = json.load(fp)
        
    if len(d['N_seeded_neurons']) != 3:
        print("Check number of seeded pathways!")
        raise SystemExit()
    else:
        if merge == True:
            if hemi_idx == 0:
                pathways_all = ['M1_capsule_rh']
            else:
                pathways_all = ['M1_capsule_lh']
            
            # I used origNum, but actually maybe I should only consider seeded axons (these include damaged!)
            axons_in_paths = [d['N_seeded_neurons'][0]+d['N_seeded_neurons'][1]+d['N_seeded_neurons'][2]]
        else:
            
            if hemi_idx == 0:
                pathways_all = ['tracts_capsule_face_rh','tracts_capsule_lower_rh','tracts_capsule_upper_rh']
            else:
                pathways_all = ['tracts_capsule_face_lh','tracts_capsule_lower_lh','tracts_capsule_upper_lh']
            
            # I used origNum, but actually maybe I should only consider seeded axons (these include damaged!)
            axons_in_paths = [d['N_seeded_neurons'][0],d['N_seeded_neurons'][1],d['N_seeded_neurons'][2]]
        
    return pathways_all, axons_in_paths
...
class DataProcessor:
    ...
    def __init__(self, stim_dir: str, hemi_idx: int):
        ...
        self.res_folder = os.path.join(stim_dir, 'Results_ANN')
    ...
    @staticmethod
    def _load_activation_results(res_folder: str, pathway: str, currents_shape: Tuple[int, int], damage_correction=True) -> np.ndarray:
            ...
            if os.path.isfile(pathway_file):
                with open(pathway_file, 'r') as fp:
                    ...
                    if damage_correction:
                        activation_results[prot_i, 0] = 0.01 * pam_res_dict['percent_activated'] /(0.01*(100.0 - pam_res_dict['percent_damaged']))
                    else:
                        activation_results[prot_i, 0] = 0.01 * pam_res_dict['percent_activated']
                    
        return activation_results

    def _load_and_combine_activation_results(self,pathways: list, currents_shape: Tuple[int, int], axons_per_pathway, damage_correction=True) -> np.ndarray:
        """Loads activation results for each pathway and computes one total activation.

        Args:
            pathways: Name of pathways to combine for the ANN training.
            currents_shape: Shape of the currents array (number of protocols, number of contacts).

        Returns:
            A NumPy array containing activation results (percentage activated) for each protocol and pathway.
        """
        #print(self.res_folder,pathways,currents_shape,axons_per_pathway)
        ...
        for prot_i in range(currents_shape[0]):
            ...
            N_activated = np.zeros(len(pathways),int)
            N_damaged = np.zeros(len(pathways),int)
            
            for inx, pathway in enumerate(pathways):
            
                pathway_file = os.path.join(self.res_folder, f'Pathway_status_{pathway}_{prot_i}.json')
                if os.path.isfile(pathway_file):
                    with open(pathway_file, 'r') as fp:
                        pam_res_dict = json.load(fp)
                        # Convert to fraction (0.0 to 1.0)
                        
                        N_activated[inx] = round(axons_per_pathway[inx] * 0.01 * pam_res_dict['percent_activated'])
                        N_damaged[inx] = round(axons_per_pathway[inx] * 0.01 * pam_res_dict['percent_damaged'])
                        
            if damage_correction:
                activation_results[prot_i, 0] = sum(N_activated) / sum(np.array(axons_per_pathway) - N_damaged);
            else:
                activation_results[prot_i, 0] = sum(N_activated) / sum(axons_per_pathway)
    ...
    def load_data(self, pathway: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[str], np.ndarray]:
        ...
        Args:
            pathway: Name of the pathway for the ANN training.
        """
        currents_file = os.path.join('/home/interscan/Documents/data/CapsuleWBG/Current_protocols_0.csv')
        ...
        axon_parameters_file = os.path.join(self.stim_dir, 'Results_ANN', 'Allocated_axons_parameters.json')
        all_pathways, axons_in_paths = load_from_json(axon_parameters_file,self.hemi_idx)
        pathway_idx = all_pathways.index(pathway)
        axons_in_path = axons_in_paths[pathway_idx]
        ...
        all_pathways_sep, axons_in_paths_sep = load_from_json(axon_parameters_file,self.hemi_idx,merge=False)
        activation_results = self._load_and_combine_activation_results(all_pathways_sep,currents_A.shape,axons_in_paths_sep)
        ...
        # only fit what was simulated in OSS
        if axons_in_path < MIN_AXON_NUMBER:
            print("#==============================#")
            print(pathway,axons_in_path)
            return X_train, X_test, np.zeros((0,0)), None, [], axons_in_path
        else:
            pathway = [pathway]
            return X_train, X_test, y_train, y_test, pathway, axons_in_path
...
class PathwayApproximator:
    ...
    def __init__(self, stim_dir: str, hemi_idx: int, pathway: str, check_trivial: bool, vat_recruit: bool = False):
        ...
        axon_parameters_file = os.path.join(self.stim_dir, 'Results_ANN', 'Allocated_axons_parameters.json')
        self.all_pathways, self.axons_in_path = load_from_json(axon_parameters_file, hemi_idx)
    ...
    def run(self) -> Optional[List[str]]:
        ...
        # print(np.min(X_train_augmented), np.max(y_train_augmented))
        ...
        print(f"Number of axons: {axons_in_path}")
        ...
        def evaluate_performance(model, X_test, y_test):
            ...
            print("MAX ABS ERROR: ",np.max(absolute_errors)*100, "%")
            relative_errors = np.abs((predictions - y_test) / y_test) * 100
            above_threshold_count = np.sum(absolute_errors > VALID_ERROR_THRESHOLD)
        ...
        num_layers = 1
        neurons_options = [128, 512, 1024]
        input_neurons_options = [128,512]
        ...
        for config in parameter_configs:
            ...
            with open(csv_filename, 'a') as csvfile:
                ...
                performance = evaluate_performance(model, X_validation, y_validation)
                row = [
                    config['name'],
                ...
                writer.writerow(row)
                print(row)
                print(f"Model '{config['name']}' - % of validation samples with > {VALID_ERROR_THRESHOLD*100.0}% absolute error: {performance}\n")
...
if __name__ == '__main__':
    ...
    try:
        ...
        # load from json        
        axon_parameters_file = os.path.join(stim_dir, 'Results_ANN', 'Allocated_axons_parameters.json')
        all_pathways, _ = load_from_json(axon_parameters_file, hemi_idx)
    ...
    for pathway in all_pathways:
        ...
        if ('_lh' in pathway and hemi_idx == 0) or ('_rh' in pathway and hemi_idx == 1):
            continue
