...
class PathwayApproximator:
    ...
    def run(self) -> Optional[List[str]]:
        ...
        X_train_validation, X_mono_I, y_train_validation, y_mono_I, pathway_filtered, axons_in_path = self.data_processor.load_data(self.pathway)
        ...
        pathway_activation_file = os.path.join(self.stim_dir,self.pathway + '_percent_activations_Current_protocols'+ HEMI_SUFFIX[self.hemi_idx] + '_9999_6057.npz')        
        if os.path.isfile(pathway_activation_file):
            pathway_act_dict = np.load(pathway_activation_file)
            X_test, _, y_test, _, pathway_filtered, axons_in_path = pathway_act_dict['X_train_validation'], pathway_act_dict['X_test'], pathway_act_dict['y_train_validation'], pathway_act_dict['y_test'], pathway_act_dict['pathway_filtered'], pathway_act_dict['axons_in_path']
        else:
            return None
        
        # # # # alternatively, load from a single file
        # pathway_activation_file = os.path.join(self.stim_dir,self.pathway + '_percent_activations_Current_protocols'+ HEMI_SUFFIX[self.hemi_idx] + '_10056_100.npz')        
        # if os.path.isfile(pathway_activation_file):
        #     pathway_act_dict = np.load(pathway_activation_file)
        #     X_II, X_mono_II, y_II, y_mono_II, pathway_filtered, axons_in_path = pathway_act_dict['X_train'], pathway_act_dict['X_test'], pathway_act_dict['y_train_filtered'], pathway_act_dict['y_test_filtered'], pathway_act_dict['pathway_filtered'], pathway_act_dict['axons_in_path']
        # else:
        #     return None
        ...
        X_test = np.concatenate((X_test,X_mono_I))
        y_test = np.concatenate((y_test,y_mono_I))
        ...
        # X_train = np.concatenate((X_train_validation[:int(X_train_validation.shape[0]*0.6),:],X_II[:int(X_II.shape[0]*0.6),:]))
        # y_train = np.concatenate((y_train_validation[:int(X_train_validation.shape[0]*0.6),:],y_II[:int(X_II.shape[0]*0.6),:]))

        # X_validation = np.concatenate((X_train_validation[int(X_train_validation.shape[0]*0.6):int(X_train_validation.shape[0]*0.8),:],X_II[int(X_II.shape[0]*0.6):int(X_train_validation.shape[0]*0.8),:]))
        # y_validation = np.concatenate((y_train_validation[int(X_train_validation.shape[0]*0.6):int(X_train_validation.shape[0]*0.8),:],y_II[int(X_II.shape[0]*0.6):int(X_train_validation.shape[0]*0.8),:]))

        # X_test = np.concatenate((X_train_validation[int(X_train_validation.shape[0]*0.8):,:],X_II[int(X_II.shape[0]*0.8):,:],X_mono_I))
        # y_test = np.concatenate((y_train_validation[int(X_train_validation.shape[0]*0.8):,:],y_II[int(X_II.shape[0]*0.8):,:],y_mono_I))
        
        # # split validation and test
        # X_validation, X_test = X_train_validation[:int(X_train_validation.shape[0]/2),:], X_train_validation[int(X_train_validation.shape[0]/2):,:]
        # y_validation, y_test_filtered = y_train_validation[:int(X_train_validation.shape[0]/2),:], y_train_validation[int(X_train_validation.shape[0]/2):,:]
        ...
        num_layers = 1
        neurons_options = [1024]
        #neurons_options = [1024]
        input_neurons_options = [256]
        epochs_options = [1000]
...
if __name__ == '__main__':
    ...
    for pathway in all_pathways:
        ...
        # if pathway != 'tracts_capsule_upper_rh':
        #     continue
