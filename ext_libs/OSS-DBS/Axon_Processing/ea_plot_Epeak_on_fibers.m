function ea_plot_Epeak_on_fibers(varargin)

    %ea_plot_Epeak_on_fibers('/home/netstim/leaddbs_git/connectomes/dMRI/HCP 985 (Li 2020) 3K_downsampled_by_5/data.mat','/media/netstim/Konstantin/OCD/derivatives/leaddbs/sub-CologneAlic02/miscellaneous/HCP 985 (Li 2020) 3K_downsampled_by_5/gs_20200317121650_lh/E_peak.mat',500)

    connectome = varargin{1};
    load(connectome);

    Epeak_file = varargin{2};
    E_proj = load(Epeak_file);
...
%     end
    ...
    [maxvals,myfibs] = maxk(E_proj.E_peak,numfibers);
...
%     end
    ...
    exp_norm_Epeak = (exp(E_proj.E_peak)-1.0)/max(exp(E_proj.E_peak));

    norm_Epeak = (E_proj.E_peak)/max(E_proj.E_peak);
    ...
    for fiber_i = 1:size(myfibs,1)
        ...
        if E_proj.E_peak(myfibs(fiber_i)) == 0
            continue
