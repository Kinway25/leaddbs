...
#                if np.any((eigVals*Sigma_iso_low)<Sigma_iso_lowerBoundary):
            ...
            #tmp[i, j, k, :] = np.array([tensor[0][0], tensor[1][0], tensor[2][0], tensor[1][1], tensor[2][1], tensor[2][
            #    2]])  # we need to have it as xx,yx,zx,yy,zy,zz (which is FSL standard saving procedure of DTI)
