'''
This function computes and plots percent activation for each pathway.
Original number of fibers per pathway (before Kuncel-VTA prefiltering!)

Only used for single protocol computations
'''
...
def plot_PAM_results(d):
    ...
    if 'Multi-Tract' in Connectome_name:
        ...
        number_original = []
        name_original = []
        # run this in loop, later you will check if you actually have this pathway in Summary_status.h5
    ...
    for pop_i in range(len(lst)):
        ...
        index_pathway = name_original.index(lst[pop_i]) # original index
    ...
    plt.grid()
    fig.tight_layout()
    plt.savefig(os.environ['PATIENTDIR'] + '/Images/Percent_activation_profile_'+str(d['Stim_side'])+'.png', format='png',
                        dpi=1000)
