...
for side = 1:numSide
    ...
    for pt = 1:numPatient
        ...
        if pt <= length(obj.allpatients) / 2
            % right side solutions are just loaded   
            ...
            % left side solutions need to be flipped to the right
            if side == 1
                % remove "_from_left" from the patients name
                E_proj_folder = [obj.allpatients{pt}(1:end-10),filesep,'miscellaneous',filesep,obj.connectome,filesep,'gs_', obj.M.guid,'_lh'];
            else
                E_proj_folder = [obj.allpatients{pt},filesep,'miscellaneous',filesep,obj.connectome,filesep,'gs_', obj.M.guid,'_rh'];
                ea_warndlg("Should not be here")
                return
            end
