...
mkdir([options.subj.stimDir, filesep, ea_nt(options.native), S.label]);
copyfile('/home/interscan/Documents/archives/Postop/PostOpBIDS/derivatives/leaddbs/sub-SBN9F7S6/stimulations/native/gs_Atlas/Current_protocols_0.csv',[options.subj.stimDir, filesep, ea_nt(options.native), S.label, filesep, 'Current_protocols_0.csv']);
copyfile('/home/interscan/Documents/archives/Postop/PostOpBIDS/derivatives/leaddbs/sub-SBN9F7S6/stimulations/native/gs_Atlas/Current_protocols_1.csv',[options.subj.stimDir, filesep, ea_nt(options.native), S.label,filesep, 'Current_protocols_1.csv']);
    ...
    for side = 0:1
        ...
        for i = 1:settings.N_samples  % mutiple samples if probablistic PAM is used, otherwise 1
            ...
            if settings.prob_PAM && settings.stimSetMode    
                ...
                mkdir([outputPaths.HemiSimFolder,filesep,'sample_diam_',num2str(i)])
                for act_file_inx = 1:length(act_files)
                    ...
                    if ~act_files(act_file_inx).isdir
                        ...
                        destFile = fullfile([outputPaths.HemiSimFolder,filesep,'sample_diam_',num2str(i)], fileName);
